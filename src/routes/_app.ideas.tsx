...
function IdeasPage() {
  ...
  const submit = () => {
    ...
    createIdea.mutate(
      ...
      {
        onSuccess: () => { setTitle(""); setDesc(""); toast.success("Nouvelle idée pitchée 🎬"); },
  ...
  const onVote = (id: string, kind: "like" | "dislike", currentVote: "like" | "dislike" | null | undefined) => {
    voteIdea.mutate(
      { id, kind: currentVote === kind ? "clear" : kind },
            ...
            <div key={i.id}
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-2">
                  <button onClick={() => onVote(i.id, "like", i.my_vote)} disabled={voteIdea.isPending}
                  ...
                  <button onClick={() => onVote(i.id, "dislike", i.my_vote)} disabled={voteIdea.isPending}
                    className={`flex items-center gap-1 text-xs px-3 py-1.5 rounded-full border transition ${
                ...
                <select value={i.status} onChange={(e) => setStatus.mutate({ id: i.id, status: e.target.value as IdeaStatus })}
